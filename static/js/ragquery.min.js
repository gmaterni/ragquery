...
function showPrompts(a){if(0!=Rag.prompts.length){for(const b of Rag.prompts)console.log(b);a=Rag.prompts.map((b,c)=>`[${c+1}]${b}\n`).join("\n");wnds.wpre.show(a)}};const ClientLLM=a=>{let b=null,c=!1;const d=(l,g=null,h=null,m=null)=>({ok:l,response:g,data:h,error:m}),e=(l,g,h,m)=>({message:l||null,type:g||null,code:h||null,details:{message:m?.message||null,type:m?.type||null,param:m?.param||null,code:m?.code||null}}),f=async l=>{let g,h="HTTPError",m={400:"Richiesta non valida",401:"Non autorizzato - Controlla la API key",403:"Accesso negato",404:"Endpoint non trovato",429:"Troppe richieste - Rate limit superato",500:"Errore interno del server",503:"Servizio non disponibile"}[l.status]||
`Errore HTTP ${l.status}`;try{if(l.headers.get("Content-Type")?.includes("application/json")){if(g=await l.json(),400===l.status&&g){const n="string"===typeof g.error?g.error:g.message||g.error?.message;n&&(n.includes("token limit")||n.includes("token exceeded")||n.includes("input too long")||n.includes("context length")||n.includes("max tokens"))&&(m="Input troppo lungo - Superato il limite di token",h="TokenLimitError")}}else g=await l.text(),400===l.status&&(g.includes("token limit")||g.includes("input too long")||
g.includes("context length"))&&(m="Input troppo lungo - Superato il limite di token",h="TokenLimitError")}catch(n){g={message:"Impossibile estrarre i dettagli dell'errore"}}return e(m,h,l.status,"string"===typeof g?{message:g}:g)},k=l=>"AbortError"===l.name?c?e("Richiesta annullata dall'utente","CancellationError",499,{message:"La richiesta \u00e8 stata interrotta volontariamente dall'utente"}):e("Richiesta interrotta per timeout","TimeoutError",408,{message:"La richiesta \u00e8 stata interrotta a causa di un timeout",
isTimeout:!0}):"TypeError"===l.name&&l.message.includes("Failed to fetch")?e("Errore di rete","NetworkError",0,{message:"Impossibile raggiungere il server. Controlla la connessione."}):e("Errore imprevisto",l.name||"UnknownError",500,{message:l.message||"Si \u00e8 verificato un errore sconosciuto"});return{sendRequest:async(l,g,h=60)=>{c=!1;b=new AbortController;h=setTimeout(()=>{b&&b.abort()},1E3*h);try{const m=await fetch(l,{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},
body:JSON.stringify(g),signal:b.signal});if(c){const q=e("Richiesta annullata","CancellationError",499,{message:"La richiesta \u00e8 stata interrotta volontariamente dall'utente"});return d(!1,null,null,q)}if(!m.ok){const q=await f(m);return d(!1,null,null,q)}const n=await m.json();return d(!0,n)}catch(m){const n=k(m);return d(!1,null,null,n)}finally{clearTimeout(h),b=null}},createError:e,cancelRequest:()=>{c=!0;return b?(b.abort(),b=null,!0):!1}}};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
...
raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,this.answers)},readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.saveToDb();
var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const k=DataMgr.doc_names[e],l=f.length;xlog(`${k} (${l}) `);UaLog.log(`${k} (${l}) `);++b;var d=1;let g=0,h="",m="",n="",q="",v=[];for(;;){const u=getPartSize(f,promptDoc("",a,""),g);if(10>u)break;[m,n]=getPartDoc(f,u);ragLog(`${c}) ${b},${d}`,m.length,n.length,this.answers);h=promptDoc(m,a,k);this.addPrompt(h);const t=getPayloadDoc(h),p=await getResponse(MODEL,t,90);if(!p)return"";const r=p.error;
if(!p.ok){console.error("ERR1\n",r);const z=r.code;if(400==z)if(isTooLarge(r)){UaLog.log(`Error tokens Doc ${h.length}`);g+=PROMPT_DECR;continue}else throw r;else if(408==z){UaLog.log("Error timeout Context");continue}else throw r;}q=p.data;if(!q)return"";let w=calcTokens.get_sum_input_tokens(),x=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${w} ${x}`);const A=p.response;responseDetails.set(A);w=responseDetails.get_total_tokens();x=responseDetails.get_completion_tokens();console.log(`Response Tokens: ${w} ${x}`);
calcTokens.add(A);d++;c++;f=n;q=cleanResponse(q);v.push(q);this.answers.push(`DOCUMENTO : ${k}_${d}\n${q}`)}const B=v.length;let y=v.join("\n\n");for(d="";;){h=promptBuildContext(y,this.ragQuery);const u=getPayloadBuildContext(h),t=await getResponse(MODEL,u,90);if(!t)return"";const p=t.error;if(!t.ok){console.error("ERR2\n",p);const r=p.code;if(400==r)if(isTooLarge(p)){UaLog.log(`Error tokens build Context ${h.length}`);y=truncateInput(y,PROMPT_DECR);continue}else throw p;else if(408==r){UaLog.log("Error timeout Context");
continue}else throw p;}d=t.data;if(!d)return"";calcTokens.add(t.response);break}UaLog.log(`context  ${B} => ${d.length}`);d=`\n### DOCUMENTO: ${k}\n ${d}`;this.docContextLst.push(d)}}catch(e){throw console.error("ERR3\n",e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();b="";c=this.ragContext;try{for(;;){let k=promptWithContext(c,a);const l=getPayloadWithContext(k),g=await getResponse(MODEL,l,90);if(!g)return"";const h=g.error;if(!g.ok){console.error("ERR4\n",h);const m=h.code;
if(400==m)if(isTooLarge(h)){UaLog.log(`Error tokens with Context ${k.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw h;else if(408==m){UaLog.log("Error timeout Context");continue}else throw h;}b=g.data;if(!b)return"";calcTokens.add(g.response);break}this.ragAnswer=b=cleanResponse(b);this.saveRespToDb();ThreadMgr.init();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);const e=calcTokens.get_sum_input_tokens(),f=calcTokens.get_sum_generate_tokens();UaLog.log(`Tokens: ${e} ${f}`);
return b}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";this.ragContext||(this.ragContext="Sei un assistente AI dispoibile a soddisfare tutte le mi richieste");if(ThreadMgr.isFirst()){ThreadMgr.init();try{var c=this.ragContext;let d=ThreadMgr.getThread();for(;;){prompt=promptThread(c,d,a);const e=getPayloadThread(prompt),f=await getResponse(MODEL,e,90);if(!f)return"";const k=f.error;if(!f.ok){console.error("ERR6\n",k);const m=k.code;if(400==m)if(isTooLarge(k)){UaLog.log(`Error tokens with Context ${prompt.length}`);
c=truncateInput(c,PROMPT_DECR);continue}else throw k;else if(408==m)continue;else throw k;}b=f.data;if(!b)return"";const l=f.response;let g=calcTokens.get_sum_input_tokens(),h=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${g} ${h}`);responseDetails.set(l);g=responseDetails.get_total_tokens();h=responseDetails.get_completion_tokens();console.log(`Response Tokens: ${g} ${h}`);calcTokens.add(l);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Inizio Conversazione (${prompt.length})`);
return b}catch(d){throw console.error("ERR7\n",d),d;}}else try{let d=this.ragContext,e=ThreadMgr.getThread();for(c="";;){c=promptThread(d,e,a);const f=getPayloadThread(c),k=await getResponse(MODEL,f,90);if(!k)return"";const l=k.error;if(!k.ok){console.error("ERR8\n",l);const n=l.code;if(400==n)if(isTooLarge(l)){UaLog.log(`Error tokens with Context ${c.length}`);d=truncateInput(d,PROMPT_DECR);continue}else throw l;else if(408==n){UaLog.log("Error timeout Context");continue}else throw l;}b=k.data;if(!b)return"";
const g=k.response;let h=calcTokens.get_sum_input_tokens(),m=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${h} ${m}`);responseDetails.set(g);h=responseDetails.get_total_tokens();m=responseDetails.get_completion_tokens();console.log(`Response Tokens: ${h} ${m}`);calcTokens.add(g);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Conversazione  (${c.length})`);return b}catch(d){throw console.error("ERR9\n",d),d;}}},LLM="# Assistant:",USER="# User:",ThreadMgr=
...
async load(){alert("load")},log(){UaLog.toggle()}},setOutText=a=>{a=answerFormtter(a);const b=document.querySelector("#id-text-out .div-text");b.innerHTML=a;b.scrollTop=b.scrollHeight},TextInput={wnd:null,init(){this.inp=document.querySelector(".text-input");document.addEventListener("keydown",a=>{document.activeElement!==this.inp&&("F1 F2 F3 F4 F5 F6 F7 F8 F9 F10 F11 F12 Control Alt Shift Meta CapsLock Escape PrintScreen ScrollLock Pause Insert Delete Home End PageUp PageDown ArrowLeft ArrowRight ArrowUp ArrowDown".split(" ").includes(a.key)||
