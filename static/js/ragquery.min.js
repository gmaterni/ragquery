...
var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const k=DataMgr.doc_names[e],l=f.length;xlog(`${k} (${l}) `);UaLog.log(`${k} (${l}) `);++b;var d=1;let g=0,h="",m="",n="",q="",v=[];for(;;){const u=getPartSize(f,promptDoc("",a,""),g);if(10>u)break;[m,n]=getPartDoc(f,u);ragLog(`${c}) ${b},${d}`,m.length,n.length,this.answers);h=promptDoc(m,a,k);this.addPrompt(h);const t=getPayloadDoc(h),p=await getResponse(t,90);if(!p)return"";const r=p.error;
