...
b);a=a.substring(b).trim();return[c,a]},ragLog=(a,b,c,d)=>{const e=MAX_PROMPT_LENGTH;d=d.reduce((f,h)=>f+h.length,0);xlog(`${a} mx:${e} lft:${b} rgt:${c} arr:${d}`);a=formatRow([a,b,c,d],[8,-7,-7,-7]);UaLog.log(a)},Rag={ragContext:"",ragQuery:"",ragPrompt:"",ragAnswer:"",answers:[],docContextLst:[],prompts:[],init(){ThreadMgr.init();this.readRespsFromDb();this.readFromDb();calcTokens.init()},returnOk(){return 10<this.ragContext.length},saveToDb(){UaDb.saveJson(ID_RAG,{context:this.ragContext,ragquery:this.ragQuery,
ragprompt:this.ragPrompt,raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragPrompt=a.ragprompt||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,this.answers)},readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();
DataMgr.readDbDocs();this.docContextLst=[];this.ragQuery=a;this.saveToDb();var b=0;try{var c=1;for(var d=0;d<DataMgr.docs.length;d++){let f=DataMgr.docs[d];if(""==f.trim())continue;const h=DataMgr.doc_names[d],k=f.length;xlog(`${h} (${k}) `);UaLog.log(`${h} (${k}) `);++b;var e=1;let g=0,l="",m="",n="",q="",v=[];for(;;){const u=getPartSize(f,promptDoc("",a,""),g);if(10>u)break;[m,n]=getPartDoc(f,u);ragLog(`${c}) ${b},${e}`,m.length,n.length,this.answers);l=promptDoc(m,a,h);this.addPrompt(l);const t=
getPayloadDoc(l),p=await getResponse(t,90);if(!p)return"";const r=p.error;if(!p.ok){console.error("ERR1\n",r);const x=r.code;if(400==x)if(isTooLarge(r)){UaLog.log(`Error tokens Doc ${l.length}`);g+=PROMPT_DECR;continue}else throw r;else if(408==x){UaLog.log("Error timeout Context");continue}else throw r;}q=p.data;if(!q)return"";const y=p.response;showTokens(y);calcTokens.add(y);e++;c++;f=n;q=cleanResponse(q);v.push(q);this.answers.push(`DOCUMENTO : ${h}_${e}\n${q}`)}const z=v.length;let w=v.join("\n\n");
for(e="";;){l=promptBuildContext(w,this.ragQuery);const u=getPayloadBuildContext(l),t=await getResponse(u,90);if(!t)return"";const p=t.error;if(!t.ok){console.error("ERR2\n",p);const r=p.code;if(400==r)if(isTooLarge(p)){UaLog.log(`Error tokens build Context ${l.length}`);w=truncateInput(w,PROMPT_DECR);continue}else throw p;else if(408==r){UaLog.log("Error timeout Context");continue}else throw p;}e=t.data;if(!e)return"";calcTokens.add(t.response);break}UaLog.log(`context  ${z} => ${e.length}`);e=`\n### DOCUMENTO: ${h}\n ${e}`;
this.docContextLst.push(e)}}catch(f){throw console.error("ERR3\n",f),f;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();b="";c=this.ragContext;d="";try{for(;;){this.ragPrompt=d=promptWithContext(c,a);const k=getPayloadWithContext(d),g=await getResponse(k,90);if(!g)return"";const l=g.error;if(!g.ok){console.error("ERR4\n",l);const m=l.code;if(400==m)if(isTooLarge(l)){UaLog.log(`Error tokens with Context ${d.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw l;else if(408==
m){UaLog.log("Error timeout Context");continue}else throw l;}b=g.data;if(!b)return"";calcTokens.add(g.response);break}this.ragAnswer=b=cleanResponse(b);this.saveRespToDb();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);const f=calcTokens.get_sum_input_tokens(),h=calcTokens.get_sum_generate_tokens();UaLog.log(`Tokens: ${f} ${h}`);return b}catch(f){throw console.error("ERR5\n",f),f;}},async requestContext(a){let b="",c=a;if(ThreadMgr.isFirst())try{this.ragContext||(c=promptThread("Sei un assistente AI dispoibile a soddisfare tutte le mi richieste",
a));const d=ThreadMgr.getThreadRows(),e=getPayloadThreadRows(d,c);for(;;){const f=await getResponse(e,90);if(!f)return"";const h=f.error;if(!f.ok){console.error("ERR6\n",h);const g=h.code;if(400==g)throw isTooLarge(h)&&alert("Conversazione troppo lunga"),h;if(408==g)continue;else throw h;}b=f.data;if(!b)return"";const k=f.response;showTokens(k);calcTokens.add(k);break}b=cleanResponse(b);this.ragContext?(ThreadMgr.add(this.ragPrompt,this.ragAnswer),ThreadMgr.add(this.ragQuery,this.ragAnswer)):ThreadMgr.add(c,
b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Inizio Conversazione (${c.length})`);return b}catch(d){throw console.error("ERR7\n",d),d;}else try{const d=ThreadMgr.getThreadRows(),e=getPayloadThreadRows(d,a);for(;;){const f=await getResponse(e,90);if(!f)return"";const h=f.error;if(!f.ok){console.error("ERR8\n",h);const g=h.code;if(400==g)throw isTooLarge(h)&&alert("Conversazione troppo lunga"),h;if(408==g){UaLog.log("Error timeout Context");continue}else throw h;}b=f.data;if(!b)return"";
const k=f.response;showTokens(k);calcTokens.add(k);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Conversazione  (${a.length})`);return b}catch(d){throw console.error("ERR9\n",d),d;}}},ASSISTANT="# Assistant:",USER="# User:",ThreadMgr={rows:[],first:!0,init(){this.rows=[];this.first=!0},delete(){this.rows=[];this.first=!0},add(a,b){this.rows.push([a,b])},getThread(){const a=[];for(let b=1;b<this.rows.length;b++){const c=this.rows[b];a.push(`${USER}\n${c[0]}\n${ASSISTANT}\n${c[1]}\n`)}return a.join("\n")},
getThreadRows(){this.first=!1;return this.rows.filter((a,b)=>1!==b)},isFirst(){return this.first}};function getPayloadDoc(a){return{model:"",temperature:.3,max_tokens:1024,stream:!1,random_seed:42,messages:[{role:"user",content:a}],safe_prompt:!1}}function getPayloadBuildContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadWithContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}
function getPayloadThread(a){return{model:"",messages:[{role:"user",content:a}],temperature:.7,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadThreadRows(a,b){let c=[];for(let d=0;d<a.length;d++){let e=a[d][0],f=a[d][1];e&&(c.push({role:"user",content:e}),c.push({role:"assistant",content:f}))}c.push({role:"user",content:b});return{model:"",messages:c,temperature:.7,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}};function promptDoc(a,b){return`
# SISTEMA
