...
UaDb.delete(b);Rag.ragQuery="";Rag.ragContext="";Rag.ragAnswer="";Rag.answers=[];ThreadMgr.rows=[]}};async function requestGet(a){try{var b=await fetch(a,{method:"GET",headers:{"Content-Type":"text/plain;charset=UTF-8"}});if(!b.ok)throw Error(`HTTP error! status: ${b.status}`);const c=await b.arrayBuffer();return(new TextDecoder("utf-8")).decode(c)}catch(c){throw console.error(`Error in requestGet() for url: ${a}`,c),b=c.message.includes("HTTP error! status")?c.message:c.message.includes("NetworkError")?"Network error occurred":"An unknown error occurred",alert(`requestGet()\nurl: ${a}\n${b}`),c;
...
this.readRespsFromDb();this.readFromDb()},returnOk(){return 10<this.ragContext.length},saveToDb(){UaDb.saveJson(ID_RAG,{context:this.ragContext,ragquery:this.ragQuery,raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,this.answers)},readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},
addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.docContextLst=[];this.ragQuery=a;ThreadMgr.initThread();this.saveToDb();var b=0;try{let e=1;for(let f=0;f<DataMgr.docs.length;f++){let g=DataMgr.docs[f];if(""==g.trim())continue;const k=DataMgr.doc_names[f],h=g.length;xlog(`${k} (${h}) `);UaLog.log(`${k} (${h}) `);++b;var c=1;let l=0,q="",u="",n="",v=[];for(;;){const r=getPartSize(g,promptDoc("",a,""),l);if(10>r)break;[q,u]=getPartDoc(g,
r);ragLog(`${e}) ${b},${c}`,q.length,u.length,this.answers);const w=promptDoc(q,this.ragquery),p=getPayloadDoc(MODEL,w),m=await client.sendRequest(p,90);if(!m)return"";const t=m.error;if(!m.ok){console.error("ERR1\n",t);if(isTooLarge(t)){UaLog.log("Error tokens ");l+=PROMPT_DECR;continue}if(408==t.code){UaLog.log("Error timeout Context");continue}else throw t;}n=m.data;if(!n)return"";c++;e++;g=u;n=cleanResponse(n);v.push(n);this.answers.push(`DOCUMENTO : ${k}_${c}\n${n}`)}const x=v.length;let y=v.join("\n\n");
for(c="";;){const r=promptBuildContext(y,this.ragQuery),w=getPayloadBuildContext(MODEL,r),p=await client.sendRequest(w,90);if(!p)return"";const m=p.error;if(!p.ok){console.error("ERR2\n",m);if(isTooLarge(m)){UaLog.log("Error tokens ");l+=PROMPT_DECR;continue}if(408==m.code){UaLog.log("Error timeout Context");continue}else throw m;}c=p.data;if(!c)return"";break}UaLog.log(`context  ${x} => ${c.length}`);c=`\n### DOCUMENTO: ${k}\n ${c}`;this.docContextLst.push(c)}}catch(e){throw console.error("ERR3\n",
e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();a="";b=this.ragContext;try{for(;;){var d=promptWithContext(b,this.ragQuery);const e=getPayloadWithContext(MODEL,d),f=await client.sendRequest(e,90);if(!f)return"";const g=f.error;if(!f.ok){console.error("ERR4\n",g);if(isTooLarge(g)){UaLog.log("Error tokens}");decr+=PROMPT_DECR;continue}if(408==g.code){UaLog.log("Error timeout Context");continue}else throw g;}a=f.data;if(!a)return"";break}this.ragAnswer=a=cleanResponse(a);this.saveRespToDb();
this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);d=[];d.push({role:"user",content:this.ragQuery});d.push({role:"assistant",content:a});return messages2html(d)}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";ThreadMgr.isFirst()?(console.log("******** FIRST"),this.ragContext?(a=promptThreadRag(this.ragContext,this.ragQuery,this.ragAnswer,a),ThreadMgr.addMessages(a)):(a=promptThread(a),ThreadMgr.addMessages(a))):(console.log("******** NEXT"),ThreadMgr.addQuery(a));
try{const c=ThreadMgr.getMessages(),d=getPayloadThread(MODEL,c);for(;;){const g=await client.sendRequest(d,90);if(!g)return"";const k=g.error;if(!g.ok){console.error("ERR6\n",k);if(408==k.code)continue;isTooLarge(k)&&alert("Conversazione troppo lunga");throw k;}b=g.data;if(!b)return"";break}b=cleanResponse(b);ThreadMgr.addAnswer(b);const e=ThreadMgr.getUserMessages(),f=messages2html(e);UaLog.log(`Inizio Conversazione (${prompt.length})`);return f}catch(c){throw console.error("ERR7\n",c),c;}}},ThreadMgr=
{rows:[],initThread(){this.rows=[]},delete(){alert("DELETE");this.rows=[]},addMessages(a){for(const b of a)this.rows.push(b);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getMessages(){return this.rows},getUserMessages(){return this.rows.filter((a,b)=>"system"!==a.role)},addQuery(a){this.addMessages([{role:"user",content:a}])},addAnswer(a){this.addMessages([{role:"assistant",content:a}])},isFirst(){return 1>this.rows.length}};function getPayloadDoc(a,b){return{model:a,temperature:.3,max_tokens:1024,stream:!1,random_seed:42,messages:b,safe_prompt:!1}}function getPayloadBuildContext(a,b){return{model:a,messages:b,temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadWithContext(a,b){return{model:a,messages:b,temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}
