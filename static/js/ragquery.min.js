const VERS="0.1.05 (13-02-2025)";var xlog=function(){};const cancelRequest=()=>{cancelClientRequest();hideSpinner()},showSpinner=()=>{const a=document.getElementById("spinner");a.classList.add("show-spinner");a.addEventListener("click",cancelRequest)},hideSpinner=()=>{const a=document.getElementById("spinner");a.classList.remove("show-spinner");a.removeEventListener("click",cancelRequest)};
...
function showPrompts(a){if(0!=Rag.prompts.length){for(const b of Rag.prompts)console.log(b);a=Rag.prompts.map((b,c)=>`[${c+1}]${b}\n`).join("\n");wnds.wpre.show(a)}};const MistralApiClient=(a,b={})=>{const c=b.timeout||60,d=b.baseUrl||"https://api.mistral.ai/v1";let e=null;const f=async k=>{let h;try{h=await k.json()}catch{h=await k.text()}return g({message:{400:"Richiesta non valida",401:"Non autorizzato - Controlla la API key",403:"Accesso negato",404:"Endpoint non trovato",429:"Troppe richieste - Rate limit superato",500:"Errore interno del server",503:"Servizio non disponibile"}[k.status]||k.statusText,type:"HTTPError",code:k.status,details:h})},g=({message:k,
type:h,code:l,details:n})=>({message:k,type:h,code:l,details:n});return{chat:async(k,h,l=c)=>{const n=k&&"string"===typeof k?!h||"object"!==typeof h||Array.isArray(h)?g({message:"Payload non valido",type:"ValidationError",code:400,details:"Il payload deve essere un oggetto valido"}):null:g({message:"Modello non valido",type:"ValidationError",code:400,details:"Il parametro model deve essere una stringa non vuota"});if(n)return[null,n];h.model=k;e=new AbortController;k=setTimeout(()=>e.abort(),1E3*
l);try{const m=await fetch(`${d}${"/chat/completions"}`,{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},body:JSON.stringify(h),signal:e.signal});clearTimeout(k);if(!m.ok)return[null,await f(m)];const r=await m.json();return r?.choices?.[0]?.message?.content?[r,null]:[null,g({message:"Risposta non valida",type:"ParseError",code:500,details:"La risposta non contiene il contenuto atteso"})]}catch(m){clearTimeout(k);var t="AbortError"===m.name?g({message:"Richiesta interrotta per timeout",
type:"TimeoutError",code:408,details:`La richiesta ha superato il limite di ${l} secondi`}):g({message:"Errore di rete",type:"NetworkError",code:0,details:"Impossibile raggiungere il server. Controlla la connessione."});return[null,t]}finally{e=null}},cancelRequest:()=>{e&&(e.abort(),e=null)}}},infoError={set(a){this.error=a},get_message(){return this.error.message},get_type(){return this.error.type},get_code(){return this.error.code},get_details_message(){return this.error.details.message},get_details_type(){return this.error.details.type},
get_details_param(){return this.error.details.param},get_details_code(){return this.error.details.code}},infoResponse={set(a){this.response=a},get_id(){return this.response.id},get_created(){return this.response.created},get_model(){return this.response.model},get_index(){return this.response.choices[0].index},get_role(){return this.response.choices[0].message.role},get_tool_calls(){return this.response.choices[0].message.tool_calls},get_content(){return this.response.choices[0].message.content},
get_finish_reason(){return this.response.choices[0].finish_reason},get_prompt_tokens(){return this.response.usage.prompt_tokens},get_total_tokens(){return this.response.usage.total_tokens},get_completion_tokens(){return this.response.usage.completion_tokens}},calcTokens={sum_input_tokens:0,sum_generate_tokens:0,init(){this.sum_generate_tokens=this.sum_input_tokens=0},add(a){a&&(this.sum_input_tokens+=a.usage.total_tokens,this.sum_generate_tokens+=a.usage.completion_tokens)},get_sum_input_tokens(){return this.sum_input_tokens},
get_sum_generate_tokens(){return this.sum_generate_tokens}},errorDumps=a=>{const b=JSON.stringify(a,null,2);return"{}"==b?`${a}`:b};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
...
}};const ID_RAG="id_rag",ID_THREAD="id_thread",ID_RESPONSES="id_responses",ID_DOC_NAMES="id_doc_names",ID_DOCS="id_docs",PROMPT_DECR=10240,MAX_PROMPT_LENGTH=399360,MODEL="mistral-small-2501",APIKEY="YhGMPy8ntz9wjJzacynYqOZc29RRGBFO",client=MistralApiClient(APIKEY,{timeout:90});function cancelClientRequest(){client.cancelRequest()}
const getPromptTokens=a=>(a=a.details.message.match(/Prompt contains (\d+) tokens/))?parseInt(a[1],10):null,getModelToken=a=>(a=a.details.message.match(/model with (\d+) maximum context length/))?parseInt(a[1],10):null,isTooLarge=a=>a.details.message.includes("too large"),getResponse=async(a,b)=>{try{const c=await client.chat(a,b),d=c[1];if(d)throw d;const e=c[0];return[e.choices[0].message.content,null,e]}catch(c){return console.error(`Error in getResponse: ${c}`),[null,c,null]}},truncateInput=(a,
b)=>a.substring(0,a.length-b),getPartSize=(a,b,c)=>{c=MAX_PROMPT_LENGTH-c;a.length+b.length<c?c=a.length:(a=a.indexOf(".",c),a=(-1!==a?a:c)+1,a>c+100&&(a=c),c=a);return c},getPartDoc=(a,b)=>{const c=a.substring(0,b);a=a.substring(b).trim();return[c,a]},ragLog=(a,b,c,d)=>{const e=MAX_PROMPT_LENGTH;d=d.reduce((f,g)=>f+g.length,0);xlog(`${a} mx:${e} lft:${b} rgt:${c} arr:${d}`);a=formatRow([a,b,c,d],[8,-7,-7,-7]);UaLog.log(a)},Rag={ragContext:"",ragQuery:"",ragAnswer:"",answers:[],docContextLst:[],prompts:[],
doc:"",doc_part:"",init(){this.readRespsFromDb();this.readFromDb();calcTokens.init()},returnOk(){return 10<this.ragContext.length},saveToDb(){UaDb.saveJson(ID_RAG,{context:this.ragContext,ragquery:this.ragQuery,raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,this.answers)},
readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.saveToDb();var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const g=DataMgr.doc_names[e],k=f.length;xlog(`${g} (${k}) `);UaLog.log(`${g} (${k}) `);++b;var d=1;let h=0,l="",n="",t="",m="",r=[];for(;;){const v=getPartSize(f,promptDoc("",a,""),h);if(10>v)break;
[n,t]=getPartDoc(f,v);ragLog(`${c}) ${b},${d}`,n.length,t.length,this.answers);l=promptDoc(n,a,g);this.addPrompt(l);const u=getPayloadDoc(l),p=await getResponse(MODEL,u,90),q=p[1];if(q){console.error("ERR1\n",q);const z=q.code;if(400==z)if(isTooLarge(q)){UaLog.log(`Error tokens Doc ${l.length}`);h+=PROMPT_DECR;continue}else throw q;else if(408==z){UaLog.log("Error timeout Context");continue}else throw q;}m=p[0];const A=p[2];if(!m)return"";let w=calcTokens.get_sum_input_tokens(),x=calcTokens.get_sum_generate_tokens();
console.log(`Sum Tokens: ${w} ${x}`);infoResponse.set(A);w=infoResponse.get_total_tokens();x=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${w} ${x}`);calcTokens.add(A);d++;c++;f=t;m=cleanResponse(m);r.push(m);this.answers.push(`DOCUMENTO : ${g}_${d}\n${m}`)}const B=r.length;let y=r.join("\n\n");for(d="";;){l=promptBuildContext(y,this.ragQuery);const v=getPayloadBuildContext(l),u=await getResponse(MODEL,v,90),p=u[1];if(p){console.error("ERR2\n",p);const q=p.code;if(400==q)if(isTooLarge(p)){UaLog.log(`Error tokens build Context ${l.length}`);
y=truncateInput(y,PROMPT_DECR);continue}else throw p;else if(408==q){UaLog.log("Error timeout Context");continue}else throw p;}d=u[0];if(!d)return"";calcTokens.add(u[2]);break}UaLog.log(`context  ${B} => ${d.length}`);d=`\n### DOCUMENTO: ${g}\n ${d}`;this.docContextLst.push(d)}}catch(e){throw console.error("ERR3\n",e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();b="";c=this.ragContext;try{for(;;){let g=promptWithContext(c,a);const k=getPayloadWithContext(g),h=await getResponse(MODEL,
k,90),l=h[1];if(l){console.error("ERR4\n",l);const n=l.code;if(400==n)if(isTooLarge(l)){UaLog.log(`Error tokens with Context ${g.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw l;else if(408==n){UaLog.log("Error timeout Context");continue}else throw l;}b=h[0];if(!b)return"";calcTokens.add(h[2]);break}this.ragAnswer=b=cleanResponse(b);this.saveRespToDb();ThreadMgr.init();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);const e=calcTokens.get_sum_input_tokens(),f=calcTokens.get_sum_generate_tokens();
UaLog.log(`Tokens: ${e} ${f}`);return b}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";this.ragContext||(this.ragContext="Sei un assistente AI dispoibile a soddisfare tutte le mi richieste");if(ThreadMgr.isFirst()){ThreadMgr.init();try{var c=this.ragContext;let d=ThreadMgr.getThread();for(;;){prompt=promptThread(c,d,a);const e=getPayloadThread(prompt),f=await client.chat(MODEL,e,90),g=f[1];if(g){console.error("ERR6\n",g);const k=g.code;if(400==k)if(isTooLarge(g)){UaLog.log(`Error tokens with Context ${prompt.length}`);
c=truncateInput(c,PROMPT_DECR);continue}else throw g;else if(408==k)continue;else throw g;}b=f[0];if(!b)return"";calcTokens.add(f[2]);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Inizio Conversazione (${prompt.length})`);return b}catch(d){throw console.error("ERR7\n",d),d;}}else try{let d=this.ragContext,e=ThreadMgr.getThread();for(c="";;){c=promptThread(d,e,a);const f=getPayloadThread(c),g=await client.chat(MODEL,f,90),k=g[1];if(k){console.error("ERR8\n",k);const h=
k.code;if(400==h)if(isTooLarge(k)){UaLog.log(`Error tokens with Context ${c.length}`);d=truncateInput(d,PROMPT_DECR);continue}else throw k;else if(408==h){UaLog.log("Error timeout Context");continue}else throw k;}b=g[0];if(!b)return"";calcTokens.add(g[2]);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Conversazione  (${c.length})`);return b}catch(d){throw console.error("ERR9\n",d),d;}}},LLM="## Assistant:",USER="## User:",ThreadMgr={rows:[],init(){this.rows=[];Rag.ragAnswer?
this.add(Rag.ragQuery,Rag.ragAnswer):this.add("","")},add(a,b){this.rows.push([a,b]);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getThread(){const a=[];for(const b of this.rows){const c=b[0];c&&a.push(`${USER}\n${c}\n${LLM}\n${b[1]}\n`)}return a.join("\n\n")},isFirst(){return 2>this.rows.length}};function getPayloadDoc(a){return{model:"",temperature:.3,max_tokens:512,stream:!1,random_seed:42,messages:[{role:"user",content:a}],response_format:{type:"text"},safe_prompt:!1}}function getPayloadBuildContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:0,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadWithContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:512,stream:!1,safe_prompt:!1,random_seed:42}}
function getPayloadThread(a){return{model:"",messages:[{role:"user",content:a}],temperature:.7,max_tokens:512,stream:!1,safe_prompt:!1,random_seed:42}};function promptDoc(a,b){return`
### RUOLO ###
...
### ISTRUZIONI ###
1. Analizza attentamente il contesto e la domanda: "${b}".
...
a=>this.handleEnter(a));document.querySelector(".send-input").addEventListener("click",()=>this.send());document.querySelector(".send2-input").addEventListener("click",()=>this.send2());document.querySelector(".clear-input").addEventListener("click",()=>this.clear())},handleEnter(a){"Enter"!==a.key||a.shiftKey||(a.preventDefault(),this.send2())},async send(){if(this.inp.value)if(0==DataMgr.readDbDocNames().length)alert("Non vi sono documenti da elaborare.\n  Se vuoi iniziare una conversazione usa il pulsante verde o return  ");
else{if(Rag.ragContext&&!await confirm("Vuoi iniziare una nuova elabrazione ?"))return"";showSpinner();setOutText("");var a=this.inp.value.trim();try{let b=await Rag.requestDocsRAG(a);b=cleanOut(b);setOutText(b);this.inp.value="";UaLog.close()}catch(b){console.error("ERROR Send",b),a=errorDumps(b),alert(a)}hideSpinner()}else alert("Ricorda di scrivere la Query  ")},async send2(){if(this.inp.value){showSpinner();ThreadMgr.isFirst()&&setOutText("");var a=this.inp.value.trim();try{let b=await Rag.requestContext(a);
if(""==b){hideSpinner();return}b=cleanOut(b);setOutText(b);this.inp.value=""}catch(b){console.error("Error send2",b),a=errorDumps(b),alert(a)}hideSpinner()}else alert("Ricorda di scrivere la Query  ")},async clear(){await confirm("Confermi cancellazione conversazione? ")&&(this.inp.value="",setOutText(""),ThreadMgr.init())}};
