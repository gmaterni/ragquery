...
function elencoDati(a){var b=UaDb.getAllIds();a=[];for(var c of b)b=UaDb.read(c).length,a.push(`${c} (${b})`);c=a.join("\n ");wnds.wpre.show(c)}const showT=a=>{wnds.wpre.show(DataMgr.docs[a])};function elencoDocs(){DataMgr.readDbDocs();DataMgr.readDbDocNames();var a=DataMgr.doc_names,b=UaJtfh();let c=0;b.append("<ul>");for(const k of a){a=b;var d=a.append,e=k,f=c++;d.call(a,`
...
function showPrompts(a){if(0!=Rag.prompts.length){for(const b of Rag.prompts)console.log(b);a=Rag.prompts.map((b,c)=>`[${c+1}]${b}\n`).join("\n");wnds.wpre.show(a)}};const errorDetails={set(a){this.error=a},get_message(){return this.error.message},get_type(){return this.error.type},get_code(){return this.error.code},get_details_message(){return this.error.details.message},get_details_type(){return this.error.details.type},get_details_param(){return this.error.details.param},get_details_code(){return this.error.details.code}},responseDetails={set(a){this.response=a},get_id(){return this.response.id},get_created(){return this.response.created},get_model(){return this.response.model},
get_index(){return this.response.choices[0].index},get_role(){return this.response.choices[0].message.role},get_tool_calls(){return this.response.choices[0].message.tool_calls},get_content(){return this.response.choices[0].message.content},get_finish_reason(){return this.response.choices[0].finish_reason},get_prompt_tokens(){return this.response.usage.prompt_tokens},get_total_tokens(){return this.response.usage.total_tokens},get_completion_tokens(){return this.response.usage.completion_tokens}},RequestResult=
(a=null,b=null,c=null)=>({response:a,data:b,error:c}),ClientLLM=(a,b={})=>{const c=b.timeout?1E3*b.timeout:6E4,d=b.baseUrl||"https://api.mistral.ai/v1";let e=null,f=!1;const k=(g,l)=>g&&"string"===typeof g?l&&"object"===typeof l?l.messages&&Array.isArray(l.messages)&&0!==l.messages.length?null:h("Payload incompleto","ValidationError",400,{message:"Il payload deve contenere un array 'messages' non vuoto"}):h("Payload non valido","ValidationError",400,{message:"Il payload deve essere un oggetto valido"}):
h("Modello non valido","ValidationError",400,{message:"Il parametro model deve essere una stringa non vuota"}),n=async g=>{let l,p="HTTPError",r={400:"Richiesta non valida",401:"Non autorizzato - Controlla la API key",403:"Accesso negato",404:"Endpoint non trovato",429:"Troppe richieste - Rate limit superato",500:"Errore interno del server",503:"Servizio non disponibile"}[g.status]||`Errore HTTP ${g.status}`;try{if(g.headers.get("Content-Type")?.includes("application/json")){if(l=await g.json(),400===
g.status&&l){const q="string"===typeof l.error?l.error:l.message||l.error?.message;q&&(q.includes("token limit")||q.includes("token exceeded")||q.includes("input too long")||q.includes("context length")||q.includes("max tokens"))&&(r="Input troppo lungo - Superato il limite di token",p="TokenLimitError")}}else l=await g.text(),400===g.status&&(l.includes("token limit")||l.includes("input too long")||l.includes("context length"))&&(r="Input troppo lungo - Superato il limite di token",p="TokenLimitError")}catch(q){l=
{message:"Impossibile estrarre i dettagli dell'errore"}}return h(r,p,g.status,"string"===typeof l?{message:l}:l)},m=g=>"AbortError"===g.name?h("Richiesta annullata","CancellationError",499,{message:"La richiesta \u00e8 stata interrotta volontariamente dall'utente"}):"TypeError"===g.name&&g.message.includes("Failed to fetch")?h("Errore di rete","NetworkError",0,{message:"Impossibile raggiungere il server. Controlla la connessione."}):h("Errore imprevisto",g.name||"UnknownError",500,{message:g.message||
"Si \u00e8 verificato un errore sconosciuto"}),h=(g,l,p,r)=>({message:g,type:l,code:p,details:r||{}});return{sendRequest:async(g,l,p=null)=>{f=!1;const r=k(g,l);if(r)return RequestResult(null,null,r);l.model=g;e=new AbortController;g=setTimeout(()=>{e&&(f=!0,e.abort())},null!==p?1E3*p:c);try{const q=await fetch(`${d}${"/chat/completions"}`,{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},body:JSON.stringify(l),signal:e.signal});clearTimeout(g);if(f){const u=h("Richiesta annullata",
"CancellationError",499,{message:"La richiesta \u00e8 stata interrotta volontariamente dall'utente"});return RequestResult(null,null,u)}if(!q.ok){const u=await n(q);return RequestResult(null,null,u)}const v=await q.json();if(!v.choices||!v.choices[0]||!v.choices[0].message||void 0===v.choices[0].message.content){const u=h("Risposta non valida","ParseError",500,{message:"La risposta non contiene il contenuto atteso"});return RequestResult(null,null,u)}return RequestResult(v,v.choices[0].message.content)}catch(q){return clearTimeout(g),
l=m(q),RequestResult(null,null,l)}},cancelRequest:()=>{f=!0;return e?(e.abort(),e=null,!0):!1}}};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
...
function cleanOut(a){let b=null,c="";a.split("\n").forEach(d=>{d=d.trim();d.startsWith("# User:")?(b&&(c+="</div>"),b="user",c+=`<div class="${b}"><b>User:</b>`):d.startsWith("# Assistant:")?(b&&(c+="</div>"),b="assistant",c+=`<div class="${b}"><b>Assistant:</b>`):0<d.length&&b&&(c+=`<br>${d}`)});b&&(c+="</div>");return c}
...
}};const ID_RAG="id_rag",ID_THREAD="id_thread",ID_RESPONSES="id_responses",ID_DOC_NAMES="id_doc_names",ID_DOCS="id_docs",PROMPT_DECR=10240,maxLenRequest=(a=32)=>{nc=3072*a;sp=.1*nc;return mlr=Math.trunc(nc+sp)},MAX_PROMPT_LENGTH=maxLenRequest(100),MODEL="mistral-small-2503",APIKEY="YhGMPy8ntz9wjJzacynYqOZc29RRGBFO",client=ClientLLM(APIKEY),calcTokens={sum_input_tokens:0,sum_generate_tokens:0,init(){this.sum_generate_tokens=this.sum_input_tokens=0},add(a){a&&(this.sum_input_tokens+=a.usage.total_tokens,
this.sum_generate_tokens+=a.usage.completion_tokens)},get_sum_input_tokens(){return this.sum_input_tokens},get_sum_generate_tokens(){return this.sum_generate_tokens}};function cancelClientRequest(){client.cancelRequest()}
const getPromptTokens=a=>(a=a.details.message.match(/Prompt contains (\d+) tokens/))?parseInt(a[1],10):null,getModelToken=a=>(a=a.details.message.match(/model with (\d+) maximum context length/))?parseInt(a[1],10):null,isTooLarge=a=>a.details.message.includes("too large"),getResponse=async(a,b)=>{a=await client.sendRequest(a,b);return a.error&&499===a.error.code?(alert("Request Interrotta"),null):a},truncateInput=(a,b)=>a.substring(0,a.length-b),getPartSize=(a,b,c)=>{c=MAX_PROMPT_LENGTH-c;a.length+
b.length<c?c=a.length:(a=a.indexOf(".",c),a=(-1!==a?a:c)+1,a>c+100&&(a=c),c=a);return c},getPartDoc=(a,b)=>{const c=a.substring(0,b);a=a.substring(b).trim();return[c,a]},ragLog=(a,b,c,d)=>{const e=MAX_PROMPT_LENGTH;d=d.reduce((f,k)=>f+k.length,0);xlog(`${a} mx:${e} lft:${b} rgt:${c} arr:${d}`);a=formatRow([a,b,c,d],[8,-7,-7,-7]);UaLog.log(a)},Rag={ragContext:"",ragQuery:"",ragAnswer:"",answers:[],docContextLst:[],prompts:[],doc:"",doc_part:"",init(){this.readRespsFromDb();this.readFromDb();calcTokens.init()},
returnOk(){return 10<this.ragContext.length},saveToDb(){UaDb.saveJson(ID_RAG,{context:this.ragContext,ragquery:this.ragQuery,raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,this.answers)},readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},
async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.saveToDb();var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const k=DataMgr.doc_names[e],n=f.length;xlog(`${k} (${n}) `);UaLog.log(`${k} (${n}) `);++b;var d=1;let m=0,h="",g="",l="",p="",r=[];for(;;){const u=getPartSize(f,promptDoc("",a,""),m);if(10>u)break;[g,l]=getPartDoc(f,u);ragLog(`${c}) ${b},${d}`,g.length,l.length,this.answers);
h=promptDoc(g,a,k);this.addPrompt(h);const x=getPayloadDoc(h),t=await getResponse(MODEL,x,90);if(!t)return"";const w=t.error;if(w){console.error("ERR1\n",w);const A=w.code;if(400==A)if(isTooLarge(w)){UaLog.log(`Error tokens Doc ${h.length}`);m+=PROMPT_DECR;continue}else throw w;else if(408==A){UaLog.log("Error timeout Context");continue}else throw w;}p=t.data;const B=t.response;if(!p)return"";let y=calcTokens.get_sum_input_tokens(),z=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${y} ${z}`);
responseDetails.set(B);y=responseDetails.get_total_tokens();z=responseDetails.get_completion_tokens();console.log(`Response Tokens: ${y} ${z}`);calcTokens.add(B);d++;c++;f=l;p=cleanResponse(p);r.push(p);this.answers.push(`DOCUMENTO : ${k}_${d}\n${p}`)}const q=r.length;let v=r.join("\n\n");for(d="";;){h=promptBuildContext(v,this.ragQuery);const u=getPayloadBuildContext(h),x=await getResponse(MODEL,u,90);if(!x)return"";const t=x.error;if(t){console.error("ERR2\n",t);const w=t.code;if(400==w)if(isTooLarge(t)){UaLog.log(`Error tokens build Context ${h.length}`);
v=truncateInput(v,PROMPT_DECR);continue}else throw t;else if(408==w){UaLog.log("Error timeout Context");continue}else throw t;}d=x.data;if(!d)return"";calcTokens.add(x.response);break}UaLog.log(`context  ${q} => ${d.length}`);d=`\n### DOCUMENTO: ${k}\n ${d}`;this.docContextLst.push(d)}}catch(e){throw console.error("ERR3\n",e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();b="";c=this.ragContext;try{for(;;){let k=promptWithContext(c,a);const n=getPayloadWithContext(k),m=await getResponse(MODEL,
n,90);if(!m)return"";const h=m.error;if(h){console.error("ERR4\n",h);const g=h.code;if(400==g)if(isTooLarge(h)){UaLog.log(`Error tokens with Context ${k.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw h;else if(408==g){UaLog.log("Error timeout Context");continue}else throw h;}b=m.data;if(!b)return"";calcTokens.add(m.response);break}this.ragAnswer=b=cleanResponse(b);this.saveRespToDb();ThreadMgr.init();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);const e=calcTokens.get_sum_input_tokens(),
f=calcTokens.get_sum_generate_tokens();UaLog.log(`Tokens: ${e} ${f}`);return b}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";this.ragContext||(this.ragContext="Sei un assistente AI dispoibile a soddisfare tutte le mi richieste");if(ThreadMgr.isFirst()){ThreadMgr.init();try{var c=this.ragContext;let d=ThreadMgr.getThread();for(;;){prompt=promptThread(c,d,a);const e=getPayloadThread(prompt),f=await getResponse(MODEL,e,90);if(!f)return"";const k=f.error;if(k){console.error("ERR6\n",
k);const g=k.code;if(400==g)if(isTooLarge(k)){UaLog.log(`Error tokens with Context ${prompt.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw k;else if(408==g)continue;else throw k;}b=f.data;const n=f.response;if(!b)return"";let m=calcTokens.get_sum_input_tokens(),h=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${m} ${h}`);responseDetails.set(n);m=responseDetails.get_total_tokens();h=responseDetails.get_completion_tokens();console.log(`Response Tokens: ${m} ${h}`);calcTokens.add(n);
break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Inizio Conversazione (${prompt.length})`);return b}catch(d){throw console.error("ERR7\n",d),d;}}else try{let d=this.ragContext,e=ThreadMgr.getThread();for(c="";;){c=promptThread(d,e,a);const f=getPayloadThread(c),k=await getResponse(MODEL,f,90);if(!k)return"";const n=k.error;if(n){console.error("ERR8\n",n);const l=n.code;if(400==l)if(isTooLarge(n)){UaLog.log(`Error tokens with Context ${c.length}`);d=truncateInput(d,PROMPT_DECR);
continue}else throw n;else if(408==l){UaLog.log("Error timeout Context");continue}else throw n;}b=k.data;const m=k.response;if(!b)return"";let h=calcTokens.get_sum_input_tokens(),g=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${h} ${g}`);responseDetails.set(m);h=responseDetails.get_total_tokens();g=responseDetails.get_completion_tokens();console.log(`Response Tokens: ${h} ${g}`);calcTokens.add(m);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Conversazione  (${c.length})`);
return b}catch(d){throw console.error("ERR9\n",d),d;}}},LLM="# Assistant:",USER="# User:",ThreadMgr={rows:[],init(){this.rows=[];Rag.ragAnswer?this.add(Rag.ragQuery,Rag.ragAnswer):this.add("","")},add(a,b){this.rows.push([a,b]);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getThread(){const a=[];for(const b of this.rows){const c=b[0];c&&a.push(`${USER}\n${c}\n${LLM}\n${b[1]}\n`)}return a.join("\n\n")},isFirst(){return 2>this.rows.length}};function getPayloadDoc(a){return{model:"",temperature:.3,max_tokens:1024,stream:!1,random_seed:42,messages:[{role:"user",content:a}],response_format:{type:"text"},safe_prompt:!1}}function getPayloadBuildContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadWithContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}
...
{b.classList.contains("active")&&b.classList.remove("active")};UaLog.callShow=()=>{b.classList.contains("active")||b.classList.add("active")};UaLog.setXY(54,13).setZ(111).new();UaLog.log_show("")},close(){const a=document.querySelector("#menu-toggle");document.querySelector("body").classList.remove("menu-open",a.checked);document.querySelector(".menu-btn").checked=!1},async help(){const a=await requestGet("./help0.html");wnds.wdiv.show(a)},upload(){RagUpload.open()},uploadDir(){RagUpload.openDir()},
async load(){alert("load")},log(){UaLog.toggle()}},setOutText=a=>{a=cleanOut(a);const b=document.querySelector("#id-text-out .div-text");b.innerHTML=a;b.scrollTop=b.scrollHeight},TextInput={wnd:null,init(){this.inp=document.querySelector(".text-input");document.addEventListener("keydown",a=>{document.activeElement!==this.inp&&("F1 F2 F3 F4 F5 F6 F7 F8 F9 F10 F11 F12 Control Alt Shift Meta CapsLock Escape PrintScreen ScrollLock Pause Insert Delete Home End PageUp PageDown ArrowLeft ArrowRight ArrowUp ArrowDown".split(" ").includes(a.key)||
...
var UaDrag=function(a){return function(b){let c=0,d=0,e=0,f=0;const k=function(m){m=m||window.event;m.preventDefault();c=e-m.clientX;d=f-m.clientY;e=m.clientX;f=m.clientY;b.style.top=b.offsetTop-d+"px";b.style.left=b.offsetLeft-c+"px"},n=function(){document.onmouseup=null;document.onmousemove=null};b.onmousedown=function(m){m=m||window.event;let h=m.target;!(h=h||null)||nodrag_tds.includes(h.tagName.toLowerCase())||h.classList.contains(nodrag_cls)||(m.preventDefault(),e=m.clientX,f=m.clientY,document.onmouseup=
n,document.onmousemove=k)}}(a)};const UaJtfh=()=>({rows:[],init(){this.rows=[];return this},insert(a){this.rows.unshift(a);return this},append(a){this.rows.push(a);return this},text(a=""){return this.rows.join(a)},html(a=""){return this.rows.join(a).replace(/\s+|\[rn\]/g," ")}});function formatRow(a,b){return a.map((c,d)=>{d=b[d];return 0>d?c.toString().padStart(Math.abs(d)," "):c.toString().padEnd(d," ")}).join(" ")}
