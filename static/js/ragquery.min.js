...
body:JSON.stringify(g),signal:b.signal});if(c){const q=e("Richiesta annullata","CancellationError",499,{message:"La richiesta \u00e8 stata interrotta volontariamente dall'utente"});return d(!1,null,null,q)}if(!m.ok){const q=await f(m);return d(!1,null,null,q)}const n=await m.json();return d(!0,n)}catch(m){const n=l(m);return d(!1,null,null,n)}finally{clearTimeout(k),b=null}},createError:e,cancelRequest:()=>{c=!0;return b?(b.abort(),b=null,!0):!1}}};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
...
var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const l=DataMgr.doc_names[e],h=f.length;xlog(`${l} (${h}) `);UaLog.log(`${l} (${h}) `);++b;var d=1;let g=0,k="",m="",n="",q="",v=[];for(;;){const u=getPartSize(f,promptDoc("",a,""),g);if(10>u)break;[m,n]=getPartDoc(f,u);ragLog(`${c}) ${b},${d}`,m.length,n.length,this.answers);k=promptDoc(m,a,l);this.addPrompt(k);const t=getPayloadDoc(k),p=await getResponse(t,90);if(!p)return"";const r=p.error;
