...
this.send2())},async send(){if(this.inp.value)if(0==DataMgr.readDbDocNames().length)alert("Non vi sono documenti da elaborare.\n  Se vuoi iniziare una conversazione usa il pulsante verde o return  ");else{if(Rag.ragContext&&!await confirm("Vuoi iniziare una nuova elabrazione ?"))return"";showSpinner();setResponseHtml("");var a=this.inp.value.trim();try{let b=await Rag.requestDocsRAG(a);console.log("send2\n",b);setResponseHtml(b);this.inp.value="";UaLog.close()}catch(b){console.error("ERROR Send",
b),a=errorDumps(b),alert(a)}hideSpinner()}else alert("Ricorda di scrivere la Query  ")},async send2(){if(this.inp.value){showSpinner();ThreadMgr.isFirst();var a=this.inp.value.trim();try{let b=await Rag.requestContext(a);console.log("send2:\n",b);if(""==b){hideSpinner();return}setResponseHtml(b);this.inp.value=""}catch(b){console.error("Error send2",b),a=errorDumps(b),alert(a)}hideSpinner()}else alert("Ricorda di scrivere la Query  ")},async clear(){await confirm("Confermi cancellazione conversazione? ")&&
(this.inp.value="",setResponseHtml(""),ThreadMgr.delete())}};
