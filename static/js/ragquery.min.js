const VERS="0.1.08";var xlog=function(){};const cancelRequest=()=>{cancelClientRequest();hideSpinner()},showSpinner=()=>{const a=document.getElementById("spinner");a.classList.add("show-spinner");a.addEventListener("click",cancelRequest)},hideSpinner=()=>{const a=document.getElementById("spinner");a.classList.remove("show-spinner");a.removeEventListener("click",cancelRequest)};
...
function showPrompts(a){if(0!=Rag.prompts.length){for(const b of Rag.prompts)console.log(b);a=Rag.prompts.map((b,c)=>`[${c+1}]${b}\n`).join("\n");wnds.wpre.show(a)}};const MistralApiClient=(a,b={})=>{const c=b.timeout||60,d=b.baseUrl||"https://api.mistral.ai/v1";let e=null;const f=async l=>{let h;try{h=await l.json()}catch{h=await l.text()}return g({message:{400:"Richiesta non valida",401:"Non autorizzato - Controlla la API key",403:"Accesso negato",404:"Endpoint non trovato",429:"Troppe richieste - Rate limit superato",500:"Errore interno del server",503:"Servizio non disponibile"}[l.status]||l.statusText,type:"HTTPError",code:l.status,details:h})},g=({message:l,
type:h,code:k,details:m})=>({message:l,type:h,code:k,details:m});return{chat:async(l,h,k=c)=>{const m=l&&"string"===typeof l?!h||"object"!==typeof h||Array.isArray(h)?g({message:"Payload non valido",type:"ValidationError",code:400,details:"Il payload deve essere un oggetto valido"}):null:g({message:"Modello non valido",type:"ValidationError",code:400,details:"Il parametro model deve essere una stringa non vuota"});if(m)return[null,m];h.model=l;e=new AbortController;l=setTimeout(()=>e.abort(),1E3*
k);try{const n=await fetch(`${d}${"/chat/completions"}`,{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},body:JSON.stringify(h),signal:e.signal});clearTimeout(l);if(!n.ok)return[null,await f(n)];const t=await n.json();return t?.choices?.[0]?.message?.content?[t,null]:[null,g({message:"Risposta non valida",type:"ParseError",code:500,details:"La risposta non contiene il contenuto atteso"})]}catch(n){clearTimeout(l);var r="AbortError"===n.name?g({message:"Richiesta interrotta per timeout",
type:"TimeoutError",code:408,details:`La richiesta ha superato il limite di ${k} secondi`}):g({message:"Errore di rete",type:"NetworkError",code:0,details:"Impossibile raggiungere il server. Controlla la connessione."});return[null,r]}finally{e=null}},cancelRequest:()=>{e&&(e.abort(),e=null)}}},infoError={set(a){this.error=a},get_message(){return this.error.message},get_type(){return this.error.type},get_code(){return this.error.code},get_details_message(){return this.error.details.message},get_details_type(){return this.error.details.type},
...
readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.saveToDb();var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const g=DataMgr.doc_names[e],l=f.length;xlog(`${g} (${l}) `);UaLog.log(`${g} (${l}) `);++b;var d=1;let h=0,k="",m="",r="",n="",t=[];for(;;){const v=getPartSize(f,promptDoc("",a,""),h);if(10>v)break;
[m,r]=getPartDoc(f,v);ragLog(`${c}) ${b},${d}`,m.length,r.length,this.answers);k=promptDoc(m,a,g);this.addPrompt(k);const u=getPayloadDoc(k),p=await getResponse(MODEL,u,90),q=p[1];if(q){console.error("ERR1\n",q);const z=q.code;if(400==z)if(isTooLarge(q)){UaLog.log(`Error tokens Doc ${k.length}`);h+=PROMPT_DECR;continue}else throw q;else if(408==z){UaLog.log("Error timeout Context");continue}else throw q;}n=p[0];const A=p[2];if(!n)return"";let w=calcTokens.get_sum_input_tokens(),x=calcTokens.get_sum_generate_tokens();
console.log(`Sum Tokens: ${w} ${x}`);infoResponse.set(A);w=infoResponse.get_total_tokens();x=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${w} ${x}`);calcTokens.add(A);d++;c++;f=r;n=cleanResponse(n);t.push(n);this.answers.push(`DOCUMENTO : ${g}_${d}\n${n}`)}const B=t.length;let y=t.join("\n\n");for(d="";;){k=promptBuildContext(y,this.ragQuery);const v=getPayloadBuildContext(k),u=await getResponse(MODEL,v,90),p=u[1];if(p){console.error("ERR2\n",p);const q=p.code;if(400==q)if(isTooLarge(p)){UaLog.log(`Error tokens build Context ${k.length}`);
y=truncateInput(y,PROMPT_DECR);continue}else throw p;else if(408==q){UaLog.log("Error timeout Context");continue}else throw p;}d=u[0];if(!d)return"";calcTokens.add(u[2]);break}UaLog.log(`context  ${B} => ${d.length}`);d=`\n### DOCUMENTO: ${g}\n ${d}`;this.docContextLst.push(d)}}catch(e){throw console.error("ERR3\n",e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();b="";c=this.ragContext;try{for(;;){let g=promptWithContext(c,a);const l=getPayloadWithContext(g),h=await getResponse(MODEL,
l,90),k=h[1];if(k){console.error("ERR4\n",k);const m=k.code;if(400==m)if(isTooLarge(k)){UaLog.log(`Error tokens with Context ${g.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw k;else if(408==m){UaLog.log("Error timeout Context");continue}else throw k;}b=h[0];if(!b)return"";calcTokens.add(h[2]);break}this.ragAnswer=b=cleanResponse(b);this.saveRespToDb();ThreadMgr.init();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);const e=calcTokens.get_sum_input_tokens(),f=calcTokens.get_sum_generate_tokens();
UaLog.log(`Tokens: ${e} ${f}`);return b}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";this.ragContext||(this.ragContext="Sei un assistente AI dispoibile a soddisfare tutte le mi richieste");if(ThreadMgr.isFirst()){ThreadMgr.init();try{var c=this.ragContext;let d=ThreadMgr.getThread();for(;;){prompt=promptThread(c,d,a);const e=getPayloadThread(prompt),f=await getResponse(MODEL,e,90),g=f[1];if(g){console.error("ERR6\n",g);const m=g.code;if(400==m)if(isTooLarge(g)){UaLog.log(`Error tokens with Context ${prompt.length}`);
c=truncateInput(c,PROMPT_DECR);continue}else throw g;else if(408==m)continue;else throw g;}b=f[0];const l=f[2];if(!b)return"";let h=calcTokens.get_sum_input_tokens(),k=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${h} ${k}`);infoResponse.set(l);h=infoResponse.get_total_tokens();k=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${h} ${k}`);calcTokens.add(l);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Inizio Conversazione (${prompt.length})`);
return b}catch(d){throw console.error("ERR7\n",d),d;}}else try{let d=this.ragContext,e=ThreadMgr.getThread();for(c="";;){c=promptThread(d,e,a);const f=getPayloadThread(c),g=await getResponse(MODEL,f,90),l=g[1];if(l){console.error("ERR8\n",l);const r=l.code;if(400==r)if(isTooLarge(l)){UaLog.log(`Error tokens with Context ${c.length}`);d=truncateInput(d,PROMPT_DECR);continue}else throw l;else if(408==r){UaLog.log("Error timeout Context");continue}else throw l;}b=g[0];const h=g[2];if(!b)return"";let k=
calcTokens.get_sum_input_tokens(),m=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${k} ${m}`);infoResponse.set(h);k=infoResponse.get_total_tokens();m=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${k} ${m}`);calcTokens.add(h);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Conversazione  (${c.length})`);return b}catch(d){throw console.error("ERR9\n",d),d;}}},LLM="## Assistant:",USER="## User:",ThreadMgr={rows:[],init(){this.rows=
[];Rag.ragAnswer?this.add(Rag.ragQuery,Rag.ragAnswer):this.add("","")},add(a,b){this.rows.push([a,b]);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getThread(){const a=[];for(const b of this.rows){const c=b[0];c&&a.push(`${USER}\n${c}\n${LLM}\n${b[1]}\n`)}return a.join("\n\n")},isFirst(){return 2>this.rows.length}};function getPayloadDoc(a){return{model:"",temperature:.3,max_tokens:1024,stream:!1,random_seed:42,messages:[{role:"user",content:a}],response_format:{type:"text"},safe_prompt:!1}}function getPayloadBuildContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:2E3,stream:!1,safe_prompt:!1,random_seed:42}}function getPayloadWithContext(a){return{model:"",messages:[{role:"user",content:a}],temperature:.3,max_tokens:512,stream:!1,safe_prompt:!1,random_seed:42}}
