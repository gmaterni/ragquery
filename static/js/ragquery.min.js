...
calcTokens.add(h.response);break}this.ragAnswer=a=cleanResponse(a);this.saveRespToDb();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);var d=calcTokens.get_sum_input_tokens();const e=calcTokens.get_sum_generate_tokens();UaLog.log(`Tokens: ${d} ${e}`);d=[];d.push({role:"user",content:this.ragQuery});d.push({role:"assistant",content:a});return messages2html(d)}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";ThreadMgr.isFirst()?this.ragContext?(a=promptThreadRag(this.ragContext,
this.ragQuery,this.ragAnswer,this.ragQuery),ThreadMgr.addMessages(a)):(a=promptThread(a),ThreadMgr.addMessages(a)):ThreadMgr.addQuery(a);try{const c=ThreadMgr.getMessages(),d=getPayloadThread(c);for(;;){const k=await getResponse(d,90);if(!k)return"";const h=k.error;if(!k.ok){console.error("ERR6\n",h);const l=h.code;if(400==l)throw isTooLarge(h)&&alert("Conversazione troppo lunga"),h;if(408==l)continue;else throw h;}b=k.data;if(!b)return"";const f=k.response;showTokens(f);calcTokens.add(f);break}b=
cleanResponse(b);ThreadMgr.addAnswer(b);const e=ThreadMgr.getUserMessages(),g=messages2html(e);UaLog.log(`Inizio Conversazione (${prompt.length})`);return g}catch(c){throw console.error("ERR7\n",c),c;}}},ThreadMgr={rows:[],init(){this.rows=[]},delete(){this.rows=[]},addMessages(a){for(const b of a)this.rows.push(b);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getMessages(){return this.rows},getUserMessages(){return this.rows.filter((a,b)=>"system"!==a.role)},addQuery(a){this.addMessages([{role:"user",
