...
function elencoDati(a){var b=UaDb.getAllIds();a=[];for(var c of b)b=UaDb.read(c).length,a.push(`${c} (${b})`);c=a.join("\n ");wnds.wpre.show(c)}const showT=a=>{wnds.wpre.show(DataMgr.docs[a])};function elencoDocs(){DataMgr.readDbDocs();DataMgr.readDbDocNames();var a=DataMgr.doc_names,b=UaJtfh();let c=0;b.append("<ul>");for(const h of a){a=b;var d=a.append,e=h,f=c++;d.call(a,`
...
function showPrompts(a){if(0!=Rag.prompts.length){for(const b of Rag.prompts)console.log(b);a=Rag.prompts.map((b,c)=>`[${c+1}]${b}\n`).join("\n");wnds.wpre.show(a)}};const MistralApiClient=(a,b={})=>{const c=b.timeout||60,d=b.baseUrl||"https://api.mistral.ai/v1";let e=null,f=!1;const h=async g=>{let k;try{k=await g.json()}catch{k=await g.text()}return l({message:{400:"Richiesta non valida",401:"Non autorizzato - Controlla la API key",403:"Accesso negato",404:"Endpoint non trovato",429:"Troppe richieste - Rate limit superato",500:"Errore interno del server",503:"Servizio non disponibile"}[g.status]||g.statusText,type:"HTTPError",code:g.status,details:k})},l=({message:g,
type:k,code:m,details:p})=>({message:g,type:k,code:m,details:p});return{chat:async(g,k,m=c)=>{k.model=g;f=!1;e&&e.abort();e=new AbortController;g=setTimeout(()=>e.abort(),1E3*m);try{const n=await fetch(`${d}${"/chat/completions"}`,{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},body:JSON.stringify(k),signal:e.signal});if(!n.ok)return[null,await h(n)];const t=await n.json();return t?.choices?.[0]?.message?.content?[t,null]:[null,l({message:"Risposta non valida",
type:"ParseError",code:500,details:"La risposta non contiene il contenuto atteso"})]}catch(n){if(f)return null;var p="AbortError"===n.name&&f?null:"AbortError"!==n.name||f?l({message:"Errore di rete",type:"NetworkError",code:0,details:"Impossibile raggiungere il server. Controlla la connessione."}):l({message:"Richiesta interrotta per timeout",type:"TimeoutError",code:408,details:`La richiesta ha superato il limite di ${m} secondi`});return null===p?null:[null,p]}finally{clearTimeout(g),e=null,f=
!1}},cancelRequest:()=>{e&&(f=!0,e.abort(),e=null);return null}}},infoError={set(a){this.error=a},get_message(){return this.error.message},get_type(){return this.error.type},get_code(){return this.error.code},get_details_message(){return this.error.details.message},get_details_type(){return this.error.details.type},get_details_param(){return this.error.details.param},get_details_code(){return this.error.details.code}},infoResponse={set(a){this.response=a},get_id(){return this.response.id},get_created(){return this.response.created},
get_model(){return this.response.model},get_index(){return this.response.choices[0].index},get_role(){return this.response.choices[0].message.role},get_tool_calls(){return this.response.choices[0].message.tool_calls},get_content(){return this.response.choices[0].message.content},get_finish_reason(){return this.response.choices[0].finish_reason},get_prompt_tokens(){return this.response.usage.prompt_tokens},get_total_tokens(){return this.response.usage.total_tokens},get_completion_tokens(){return this.response.usage.completion_tokens}},
calcTokens={sum_input_tokens:0,sum_generate_tokens:0,init(){this.sum_generate_tokens=this.sum_input_tokens=0},add(a){a&&(this.sum_input_tokens+=a.usage.total_tokens,this.sum_generate_tokens+=a.usage.completion_tokens)},get_sum_input_tokens(){return this.sum_input_tokens},get_sum_generate_tokens(){return this.sum_generate_tokens}},errorDumps=a=>{const b=JSON.stringify(a,null,2);return"{}"==b?`${a}`:b};function removeTag(a){a=a.replace(/<<</g,"").replace(/>>>/g,"");return a=a.replace(/<</g,"").replace(/>>/g,"")}
...
}};const ID_RAG="id_rag",ID_THREAD="id_thread",ID_RESPONSES="id_responses",ID_DOC_NAMES="id_doc_names",ID_DOCS="id_docs",PROMPT_DECR=10240,maxLenRequest=(a=32)=>{nc=6144*a;sp=.15*nc;return mlr=Math.trunc(nc+sp)},MAX_PROMPT_LENGTH=maxLenRequest(32),MODEL="mistral-small-2503",APIKEY="YhGMPy8ntz9wjJzacynYqOZc29RRGBFO",client=MistralApiClient(APIKEY,{timeout:90});function cancelClientRequest(){client.cancelRequest()}
const getPromptTokens=a=>(a=a.details.message.match(/Prompt contains (\d+) tokens/))?parseInt(a[1],10):null,getModelToken=a=>(a=a.details.message.match(/model with (\d+) maximum context length/))?parseInt(a[1],10):null,isTooLarge=a=>a.details.message.includes("too large"),getResponse=async(a,b)=>{try{const c=await client.chat(a,b);if(!c)return alert("Request Interrotta"),null;const d=c[1];if(d)throw d;const e=c[0];return[e.choices[0].message.content,null,e]}catch(c){return console.error(`Error in getResponse: ${c}`),
[null,c,null]}},truncateInput=(a,b)=>a.substring(0,a.length-b),getPartSize=(a,b,c)=>{c=MAX_PROMPT_LENGTH-c;a.length+b.length<c?c=a.length:(a=a.indexOf(".",c),a=(-1!==a?a:c)+1,a>c+100&&(a=c),c=a);return c},getPartDoc=(a,b)=>{const c=a.substring(0,b);a=a.substring(b).trim();return[c,a]},ragLog=(a,b,c,d)=>{const e=MAX_PROMPT_LENGTH;d=d.reduce((f,h)=>f+h.length,0);xlog(`${a} mx:${e} lft:${b} rgt:${c} arr:${d}`);a=formatRow([a,b,c,d],[8,-7,-7,-7]);UaLog.log(a)},Rag={ragContext:"",ragQuery:"",ragAnswer:"",
answers:[],docContextLst:[],prompts:[],doc:"",doc_part:"",init(){this.readRespsFromDb();this.readFromDb();calcTokens.init()},returnOk(){return 10<this.ragContext.length},saveToDb(){UaDb.saveJson(ID_RAG,{context:this.ragContext,ragquery:this.ragQuery,raganswer:this.ragAnswer});UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},readFromDb(){const a=UaDb.readJson(ID_RAG);this.ragContext=a.context||"";this.ragQuery=a.ragquery||"";this.ragAnswer=a.raganswer||"";ThreadMgr.rows=UaDb.readArray(ID_THREAD)},saveRespToDb(){UaDb.saveArray(ID_RESPONSES,
this.answers)},readRespsFromDb(){this.answers=UaDb.readArray(ID_RESPONSES)},addPrompt(a){},async requestDocsRAG(a){DataMgr.deleteJsonDati();DataMgr.readDbDocNames();DataMgr.readDbDocs();this.ragQuery=a;this.saveToDb();var b=0;try{var c=1;for(let e=0;e<DataMgr.docs.length;e++){let f=DataMgr.docs[e];if(""==f.trim())continue;const h=DataMgr.doc_names[e],l=f.length;xlog(`${h} (${l}) `);UaLog.log(`${h} (${l}) `);++b;var d=1;let g=0,k="",m="",p="",n="",t=[];for(;;){const v=getPartSize(f,promptDoc("",a,
""),g);if(10>v)break;[m,p]=getPartDoc(f,v);ragLog(`${c}) ${b},${d}`,m.length,p.length,this.answers);k=promptDoc(m,a,h);this.addPrompt(k);const u=getPayloadDoc(k),q=await getResponse(MODEL,u,90);if(!q)return"";const r=q[1];if(r){console.error("ERR1\n",r);const z=r.code;if(400==z)if(isTooLarge(r)){UaLog.log(`Error tokens Doc ${k.length}`);g+=PROMPT_DECR;continue}else throw r;else if(408==z){UaLog.log("Error timeout Context");continue}else throw r;}n=q[0];const A=q[2];if(!n)return"";let w=calcTokens.get_sum_input_tokens(),
x=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${w} ${x}`);infoResponse.set(A);w=infoResponse.get_total_tokens();x=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${w} ${x}`);calcTokens.add(A);d++;c++;f=p;n=cleanResponse(n);t.push(n);this.answers.push(`DOCUMENTO : ${h}_${d}\n${n}`)}const B=t.length;let y=t.join("\n\n");for(d="";;){k=promptBuildContext(y,this.ragQuery);const v=getPayloadBuildContext(k),u=await getResponse(MODEL,v,90);if(!u)return"";const q=u[1];
if(q){console.error("ERR2\n",q);const r=q.code;if(400==r)if(isTooLarge(q)){UaLog.log(`Error tokens build Context ${k.length}`);y=truncateInput(y,PROMPT_DECR);continue}else throw q;else if(408==r){UaLog.log("Error timeout Context");continue}else throw q;}d=u[0];if(!d)return"";calcTokens.add(u[2]);break}UaLog.log(`context  ${B} => ${d.length}`);d=`\n### DOCUMENTO: ${h}\n ${d}`;this.docContextLst.push(d)}}catch(e){throw console.error("ERR3\n",e),e;}this.ragContext=this.docContextLst.join("\n\n");this.saveToDb();
b="";c=this.ragContext;try{for(;;){let h=promptWithContext(c,a);const l=getPayloadWithContext(h),g=await getResponse(MODEL,l,90);if(!g)return"";const k=g[1];if(k){console.error("ERR4\n",k);const m=k.code;if(400==m)if(isTooLarge(k)){UaLog.log(`Error tokens with Context ${h.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw k;else if(408==m){UaLog.log("Error timeout Context");continue}else throw k;}b=g[0];if(!b)return"";calcTokens.add(g[2]);break}this.ragAnswer=b=cleanResponse(b);this.saveRespToDb();
ThreadMgr.init();this.saveToDb();UaLog.log(`Risposta: (${this.ragAnswer.length})`);const e=calcTokens.get_sum_input_tokens(),f=calcTokens.get_sum_generate_tokens();UaLog.log(`Tokens: ${e} ${f}`);return b}catch(e){throw console.error("ERR5\n",e),e;}},async requestContext(a){let b="";this.ragContext||(this.ragContext="Sei un assistente AI dispoibile a soddisfare tutte le mi richieste");if(ThreadMgr.isFirst()){ThreadMgr.init();try{var c=this.ragContext;let d=ThreadMgr.getThread();for(;;){prompt=promptThread(c,
d,a);const e=getPayloadThread(prompt),f=await getResponse(MODEL,e,90);if(!f)return"";const h=f[1];if(h){console.error("ERR6\n",h);const m=h.code;if(400==m)if(isTooLarge(h)){UaLog.log(`Error tokens with Context ${prompt.length}`);c=truncateInput(c,PROMPT_DECR);continue}else throw h;else if(408==m)continue;else throw h;}b=f[0];const l=f[2];if(!b)return"";let g=calcTokens.get_sum_input_tokens(),k=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${g} ${k}`);infoResponse.set(l);g=infoResponse.get_total_tokens();
k=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${g} ${k}`);calcTokens.add(l);break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Inizio Conversazione (${prompt.length})`);return b}catch(d){throw console.error("ERR7\n",d),d;}}else try{let d=this.ragContext,e=ThreadMgr.getThread();for(c="";;){c=promptThread(d,e,a);const f=getPayloadThread(c),h=await getResponse(MODEL,f,90);if(!h)return"";const l=h[1];if(l){console.error("ERR8\n",l);const p=l.code;if(400==
p)if(isTooLarge(l)){UaLog.log(`Error tokens with Context ${c.length}`);d=truncateInput(d,PROMPT_DECR);continue}else throw l;else if(408==p){UaLog.log("Error timeout Context");continue}else throw l;}b=h[0];const g=h[2];if(!b)return"";let k=calcTokens.get_sum_input_tokens(),m=calcTokens.get_sum_generate_tokens();console.log(`Sum Tokens: ${k} ${m}`);infoResponse.set(g);k=infoResponse.get_total_tokens();m=infoResponse.get_completion_tokens();console.log(`Response Tokens: ${k} ${m}`);calcTokens.add(g);
break}b=cleanResponse(b);ThreadMgr.add(a,b);b=ThreadMgr.getThread();UaLog.log(`Conversazione  (${c.length})`);return b}catch(d){throw console.error("ERR9\n",d),d;}}},LLM="## Assistant:",USER="## User:",ThreadMgr={rows:[],init(){this.rows=[];Rag.ragAnswer?this.add(Rag.ragQuery,Rag.ragAnswer):this.add("","")},add(a,b){this.rows.push([a,b]);UaDb.saveArray(ID_THREAD,ThreadMgr.rows)},getThread(){const a=[];for(const b of this.rows){const c=b[0];c&&a.push(`${USER}\n${c}\n${LLM}\n${b[1]}\n`)}return a.join("\n\n")},
...
*/
const WndPre=a=>({w:UaWindowAdm.create(a),out:null,show(b){wnds.closeAll();b=`
...
async load(){alert("load")},log(){UaLog.toggle()}},setOutText=a=>{const b=document.querySelector("#id-text-out .pre-text");b.textContent=a;b.scrollTop=b.scrollHeight},TextInput={wnd:null,init(){this.inp=document.querySelector(".text-input");document.addEventListener("keydown",a=>{document.activeElement!==this.inp&&("F1 F2 F3 F4 F5 F6 F7 F8 F9 F10 F11 F12 Control Alt Shift Meta CapsLock Escape PrintScreen ScrollLock Pause Insert Delete Home End PageUp PageDown ArrowLeft ArrowRight ArrowUp ArrowDown".split(" ").includes(a.key)||
